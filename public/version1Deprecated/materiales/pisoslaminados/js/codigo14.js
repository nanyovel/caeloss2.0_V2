...
function calcular(){
  ...
  // Si el usuario selecionó un tamaño entonces quita el mensaje y calcula
    ...
    for(i=0;i<ancho_habitacion.length;i++){
      ...
      cuarto[i].zocalo = metrosL/2.4;
      cuarto[i].zocalo = Math.ceil(cuarto[i].zocalo);

      cuarto[i].bajo_piso = metros2/24.4;
      console.log(cuarto[i].bajo_piso);
      // cuarto[i].bajo_piso = Math.ceil(cuarto[i].bajo_piso);
      cuarto[i].bajo_piso = cuarto[i].bajo_piso.toFixed(2);
      cuarto[i].bajo_piso = parseFloat(cuarto[i].bajo_piso);

      if(cuarto[i].bajo_piso<0.1){
        cuarto[i].bajo_piso=0.1;
      }

      cuarto[i].silicon = cuarto[i].zocalo/2;
      cuarto[i].silicon = Math.ceil(cuarto[i].silicon);

      cuarto[i].masilla = metrosL /12;
      cuarto[i].masilla = Math.ceil(cuarto[i].masilla);

      cuarto[i].reducer = puertas[i].value / 2;
    ...
    function sumarHabitaciones(){
      ...
      totalHab.lamina = totalHab.lamina / 10;
      totalHab.lamina =Math.ceil(totalHab.lamina);
      totalHab.lamina = totalHab.lamina * 10;
      ...
      //     if(modo[2].checked == true || modo[3].checked==true){
      ...
      imprimirDato(cajaCeldaA[0], totalHab.lamina + 'unds');
