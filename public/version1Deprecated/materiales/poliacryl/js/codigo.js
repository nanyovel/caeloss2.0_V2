...
class Habitaciones{
  constructor(planchaPoli,tornillo,ancho,largo){
    this.planchaPoli = planchaPoli;
...
function calcular(){
  ...
  // Si el usuario selecionó un tamaño entonces quita el mensaje y calcula
    ...
    for(i=0;i<ancho_habitacion.length;i++){
      ...
      cuarto[i].planchaPoli = metros2 / ((planchaPoli.ancho-0.10)*planchaPoli.largo);
      cuarto[i].planchaPoli = cuarto[i].planchaPoli.toFixed(2);
      cuarto[i].planchaPoli = parseFloat(cuarto[i].planchaPoli);
      // cuarto[i].planchaPoli = Math.ceil(cuarto[i].planchaPoli);

      cuarto[i].tornillo = cuarto[i].planchaPoli * 8;
      cuarto[i].tornillo = Math.ceil(cuarto[i].tornillo);

      imprimirDato(cajaCeldaA[o], cuarto[i].planchaPoli);
      imprimirDato(cajaCeldaB[o], cuarto[i].tornillo);
    ...
    function sumarHabitaciones(){
      ...
      for(i=0; i<ancho_habitacion.length; i++){
        ...
        totalHab.planchaPoli += cuarto[i].planchaPoli;
        totalHab.tornillo += cuarto[i].tornillo;

      }
      totalHab.planchaPoli = Math.ceil(totalHab.planchaPoli);
      totalHab.tornillo = Math.ceil(totalHab.tornillo);
      ...
      for(i=0;i<ancho_habitacion.length;i++){
        ...
        imprimirDato(cajaCeldaA[0], totalHab.planchaPoli);
        imprimirDato(cajaCeldaB[0], totalHab.tornillo);
      }
