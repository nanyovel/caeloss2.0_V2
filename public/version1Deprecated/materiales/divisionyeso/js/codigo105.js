...
class Habitaciones{
  constructor(plancha4x8,parales,durmientes,esquineros,masilla5gl, tornilloPlancha, tornilloEstructura,cinta250,lija100,fulminantes,clavoYeso,ancho,largo){
...
function calcular(){
  ...
  // Si el usuario selecionó un tamaño entonces quita el mensaje y calcula
    ...
    for(i=0;i<ancho_habitacion.length;i++){
      ...
      cuarto[i].parales = (metros2/ cuarto[i].largo)/0.6;

      if(cuarto[i].largo > 3){
        cuarto[i].parales = ((metros2/ cuarto[i].largo)/0.6)*(cuarto[i].largo/3);
      ...
      cuarto[i].masilla5gl = (metros2 /25)*cara;
      ...
      cuarto[i].lija100 = (metros2/20)*cara;
      ...
      imprimirDato(cajaCeldaL[o], cuarto[i].madera);
    ...
    function sumarHabitaciones(){
      ...
      for(i=0; i<ancho_habitacion.length; i++){
        ...
        console.log(cuarto[i].plancha4x8);
...
function copiarTabla(e){
  if(calculoRealizado==true && window.screen.width>600){
    ...
    columnaCodigo.push(padre_celdaL.firstElementChild.textContent);
