...
class Habitaciones{
  constructor(listones,tornillos,sujetadores, tableroTerminacion,ancho,largo){
    this.listones = listones;
    this.tornillos = tornillos;
    this.sujetadores = sujetadores;
    this.tableroTerminacion = tableroTerminacion;
    this.ancho = ancho;
    this.largo = largo;
  }
  area2(){
...
function calcular(){
  ...
  // Si el usuario selecionó un tamaño entonces quita el mensaje y calcula
    ...
    for(i=0;i<ancho_habitacion.length;i++){
      ...
      cuarto[i].listones = metros2/0.38;
      cuarto[i].listones = Math.ceil(cuarto[i].listones);
      console.log(cuarto[i].listones );

      cuarto[i].tornillos = cuarto[i].listones * 8;
      cuarto[i].tornillos = Math.ceil(cuarto[i].tornillos);

      cuarto[i].sujetadores = cuarto[i].tornillos;
      cuarto[i].sujetadores = Math.ceil(cuarto[i].sujetadores);

      cuarto[i].tableroTerminacion = puertas[i].value / 2.9;
      cuarto[i].tableroTerminacion = parseFloat(cuarto[i].tableroTerminacion);
      cuarto[i].tableroTerminacion = cuarto[i].tableroTerminacion.toFixed(2);
      cuarto[i].tableroTerminacion = parseFloat(cuarto[i].tableroTerminacion);
      // cuarto[i].tableroTerminacion = Math.ceil(cuarto[i].tableroTerminacion);

      imprimirDato(cajaCeldaA[o], cuarto[i].listones);
      imprimirDato(cajaCeldaB[o], cuarto[i].tornillos);
      imprimirDato(cajaCeldaC[o], cuarto[i].sujetadores);
      imprimirDato(cajaCeldaD[o], cuarto[i].tableroTerminacion);
    ...
    function sumarHabitaciones(){
      totalHab.listones =0;
      totalHab.tornillos =0;
      totalHab.sujetadores=0;
      totalHab.tableroTerminacion =0;

      for(i=0; i<ancho_habitacion.length; i++){

        if(cuarto[i].listones== undefined){
          continue;
        }
        ...
        totalHab.listones += cuarto[i].listones;
        totalHab.tornillos += cuarto[i].tornillos;
        totalHab.sujetadores+= cuarto[i].sujetadores;
        totalHab.tableroTerminacion+= cuarto[i].tableroTerminacion;

        totalHab.listones = Math.ceil(totalHab.listones);
        totalHab.tornillos = Math.ceil(totalHab.tornillos);
        totalHab.sujetadores = Math.ceil(totalHab.sujetadores);
        totalHab.tableroTerminacion = Math.ceil(totalHab.tableroTerminacion);
      ...
      for(i=0;i<ancho_habitacion.length;i++){
        ...
        imprimirDato(cajaCeldaA[0], totalHab.listones);
        imprimirDato(cajaCeldaB[0], totalHab.tornillos);
        imprimirDato(cajaCeldaC[0], totalHab.sujetadores);
        imprimirDato(cajaCeldaD[0], totalHab.tableroTerminacion);
      }
